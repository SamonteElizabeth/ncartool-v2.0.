...
const ActionPlanModule: React.FC<ActionPlanModuleProps> = ({ ncars, actionPlans, setActionPlans, setNcars, role, onNotify }) => {
  ...
  const filteredNCARs = useMemo(() => {
    let base = (isLead || isAuditor)
      ? ncars.filter(n => n.status === NCARStatus.ACTION_PLAN_SUBMITTED || n.status === NCARStatus.CLOSED || n.status === NCARStatus.REOPENED || n.status === NCARStatus.REJECTED)
    ...
    return base.filter(ncar => {
      ...
      const matchesStatus = 
        ...
        (statusFilter === 'Submitted' && ncar.status === NCARStatus.ACTION_PLAN_SUBMITTED) ||
        (statusFilter === 'Open' && ncar.status === NCARStatus.OPEN) ||
  ...
  const handleSubmitAction = (e: React.FormEvent) => {
    ...
    setNcars(prev => prev.map(n => n.id === selectedNCAR.id ? { ...n, status: NCARStatus.ACTION_PLAN_SUBMITTED, rejectionRemarks: undefined } : n));
        ...
        <div className="flex gap-3 relative" ref={filterRef}>
          ...
          {showFilters && (
            <div className="absolute top-full right-0 mt-3 w-64 bg-white border border-gray-100 rounded-2xl shadow-2xl p-4 z-40">
              <span className="text-xs font-black text-gray-400 uppercase block mb-3 pb-2 border-b">Finding Types</span>
          ...
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="bg-gray-50 border-none rounded-2xl px-7 py-4 text-base font-black text-gray-600 focus:ring-2 focus:ring-blue-500 outline-none cursor-pointer">
            ...
            {isAuditee ? <><option value="Open">Open</option><option value="Reopened">Reopened</option></> : <option value="Submitted">Submitted</option>}
          </select>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#3b82f6] text-white">
                <th className="px-6 py-5 text-[12px] font-black uppercase tracking-widest">NCAR ID</th>
                <th className="px-6 py-5 text-[12px] font-black uppercase tracking-widest">Finding Type</th>
                <th className="px-6 py-5 text-[12px] font-black uppercase tracking-widest">Requirement</th>
                <th className="px-6 py-5 text-[12px] font-black uppercase tracking-widest text-center">Deadline</th>
                <th className="px-6 py-5 text-[12px] font-black uppercase tracking-widest text-center">Status</th>
                <th className="px-6 py-5 text-[12px] font-black uppercase tracking-widest text-center">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {filteredNCARs.length === 0 ? (
                <tr><td colSpan={6} className="px-8 py-12 text-center text-gray-400 font-bold italic text-[12px]">No matching items.</td></tr>
              ) : (
                filteredNCARs.map(n => (
                  <tr key={n.id} className="hover:bg-blue-50/30 transition-colors">
                    <td className="px-6 py-5"><span className="text-[12px] font-black text-blue-600 bg-blue-50 px-2 py-1 rounded border border-blue-100">{n.id}</span></td>
                    <td className="px-6 py-5"><span className={`text-[11px] font-black px-3 py-1 rounded-full uppercase border ${n.findingType === 'Major' ? 'bg-red-50 text-red-600 border-red-100' : 'bg-blue-50 text-blue-600 border-blue-100'}`}>{n.findingType}</span></td>
                    <td className="px-6 py-5"><p className="font-bold text-gray-900 text-[12px] line-clamp-1">{n.requirement}</p></td>
                    ...
                    <td className="px-6 py-5 text-center">
                      <span className={`text-[11px] font-black px-3 py-1 rounded-full uppercase border ${n.status === NCARStatus.REOPENED || n.status === NCARStatus.REJECTED ? 'bg-red-50 text-red-600 border-red-100' : n.status === NCARStatus.CLOSED ? 'bg-green-50 text-green-600 border-green-100' : n.status === NCARStatus.OPEN ? 'bg-orange-50 text-orange-600 border-orange-100' : 'bg-blue-50 text-blue-600 border-blue-100'}`}>
                        {n.status === NCARStatus.CLOSED ? 'Approved' : (n.status === NCARStatus.REOPENED ? 'Rejected' : (n.status === NCARStatus.ACTION_PLAN_SUBMITTED ? 'Review Req.' : n.status))}
                      </span>
                    </td>
                    <td className="px-6 py-5 text-center">
                      <button 
              ...
              <>
                <div className="p-8 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                  <div className="flex items-center gap-5">
                    <div className={`p-4 rounded-3xl text-white shadow-xl ${selectedNCAR.status === NCARStatus.REOPENED || selectedNCAR.status === NCARStatus.REJECTED ? 'bg-red-600' : 'bg-[#3b82f6]'}`}>
                  ...
                  <div className="space-y-8">
                    {selectedNCAR.rejectionRemarks && (
                      <div className="bg-red-50 border-2 border-red-100 rounded-2xl p-6 flex items-start gap-5 shadow-sm">
                        ...
                        <div>
                          <span className="text-[10px] font-black text-red-600 uppercase tracking-widest block mb-1">Feedback from Auditor</span>
                    ...
                    <div className="space-y-6">
                      <div><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">Statement</label><div className="bg-gray-50 p-6 rounded-3xl border border-gray-100 text-base text-gray-700 italic">"{selectedNCAR.statement}"</div></div>
                      <div><label className="text-[10px] font-black text-gray-400 uppercase block mb-2">Evidence</label><div className="bg-white p-4 rounded-xl border border-gray-100 text-sm font-bold text-gray-600 italic">{selectedNCAR.evidence}</div></div>
                    </div>
                  </div>
                  <div className="space-y-8">
                    <div className="flex items-center gap-3 text-green-600 font-black uppercase tracking-widest text-xs border-b-2 border-green-100 pb-3"><CheckCircle2 size={18} /> Plan Analysis</div>
                    <div className="space-y-6">
                ...
                <div className="p-8 border-t border-gray-100 bg-gray-50/50 flex justify-between items-center">
                  ...
                  <div className="flex gap-4">
                    {isLead && selectedNCAR.status === NCARStatus.ACTION_PLAN_SUBMITTED && (
                      <>
                        <button onClick={() => setShowRejectModal(true)} className="flex items-center gap-2 px-10 py-4 bg-white border-2 border-red-100 text-red-600 rounded-2xl font-black hover:bg-red-50 transition-all uppercase text-xs tracking-widest shadow-lg shadow-red-50"><XCircle size={18} /> Request Revision</button>
                        <button onClick={handleApprove} className="flex items-center gap-2 px-12 py-4 bg-[#3b82f6] text-white rounded-2xl font-black hover:bg-blue-700 shadow-2xl uppercase text-xs tracking-widest"><CheckCircle2 size={18} /> Approve Plan</button>
                      </>
                    )}
                    ...
                    {isAuditee && selectedNCAR.status === NCARStatus.OPEN && (
        ...
        <div className="fixed inset-0 bg-black/70 z-[60] flex items-center justify-center p-6 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl overflow-hidden border border-red-50">
            <div className="p-8 space-y-8">
              <div className="flex items-center gap-5"><div className="p-4 bg-red-50 rounded-3xl text-red-600"><AlertTriangle size={36} /></div><div><h4 className="text-2xl font-black text-gray-900 tracking-tight uppercase">Rejection Log</h4><p className="text-[10px] font-black text-gray-400 tracking-widest mt-1">Ref: {selectedNCAR?.id}</p></div></div>
              ...
              <div className="flex gap-4"><button onClick={() => { setShowRejectModal(false); setRejectionRemarks(''); }} className="flex-1 px-6 py-4 bg-gray-50 text-gray-500 rounded-2xl font-black text-xs uppercase hover:bg-gray-100 transition-all">Cancel</button><button onClick={finalizeRejection} disabled={!rejectionRemarks.trim()} className="flex-[1.5] px-6 py-4 bg-red-600 text-white rounded-2xl font-black text-xs uppercase hover:bg-red-700 shadow-2xl disabled:opacity-50 flex items-center justify-center gap-3"><AlertCircle size={18} /> Confirm Rejection</button></div>
            </div>
          </div>
        </div>
      )}
